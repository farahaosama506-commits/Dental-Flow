...
const Header: React.FC = () => {
  ...
  const handleSearch = useCallback(async (term: string) => {
    ...
    const [patientsRes, appointmentsRes] = await Promise.all([
      ...
      supabase
        .from("appointments")
        .select("id, appointment_date, appointment_time, patient:patients(full_name)")
        .eq("doctor_id", userData.user.id)
        .ilike("patient.full_name", searchPattern)
        .limit(5),
    ]);
    ...
    const results = [
      ...
      ...(appointmentsRes.data || []).map((a: any) => ({
        type: "appointment",
        id: a.id,
        title: a.patient?.full_name || "غير معروف",
        subtitle: `${a.appointment_date} - ${a.appointment_time?.substring(0, 5)}`,
        link: "/appointments",
      })),
    ];
    ...
    <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 lg:px-6 shadow-nav z-10 shrink-0">
      ...
      <div className="flex-1 max-w-xl hidden sm:block ms-10 lg:ms-0 relative">
        <div className="relative">
          ...
          />
          {searching && (
            <i className="fas fa-spinner fa-spin absolute end-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
          )}
            ...
            {searchResults.map((result, index) => (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                  result.type === "patient" ? "bg-teal-light text-teal" : "bg-green-light text-green-soft"
                }`}>
      ...
      <div className="flex items-center gap-2 sm:gap-4 ms-auto sm:ms-6">
        ...
        <Link href="/profile" className="flex items-center gap-2 sm:gap-3 ps-2 border-s border-gray-200 hover:bg-gray-50 rounded-lg px-2 py-1 transition-colors">
          <span className="text-sm font-semibold text-navy hidden sm:block hover:text-teal transition-colors">
            {doctorName}
          </span>
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-teal flex items-center justify-center text-white font-bold text-xs sm:text-sm hover:bg-teal-dark transition-colors shrink-0">
            {doctorInitials}
          </div>
        </Link>
